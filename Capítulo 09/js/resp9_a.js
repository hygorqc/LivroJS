...
const contarVisitas = () => {
  ...
  if (localStorage.getItem('contadorAcesso')) {
    contador = Number(localStorage.getItem('contadorAcesso'));
  }

  contador++;

  if (contador == 1) {
    resp.innerText = 'Bem-vindo, este é o seu primeiro acesso';
  } else {
    resp.innerText = `Bem-vindo de volta, este é o seu acesso de número ${contador}`;
  }

  localStorage.setItem('contadorAcesso', contador);
};
